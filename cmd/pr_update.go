...
func init() {
	...
	updateCmd := &cobra.Command{
		...
		Short: "Update the pull request for the current branch with an AI-generated title and body",
		Long: `Update the pull request for the current branch with an AI-generated title and body from git commits and diffs.
...
If detection fails, pass a base branch explicitly (e.g. "main") or set GH_REPO.`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			code := prcreate.RunUpdate(prcreate.Options{
				BaseBranch: base,
				BaseGiven:  baseGiven,
				Edit:       edit,
				DryRun:     dryRun,
	...
	updateCmd.Flags().BoolVarP(&edit, "edit", "e", false, "Edit the generated output in $EDITOR")
	updateCmd.Flags().BoolVarP(&dryRun, "dry-run", "n", false, "Print generated title/body only (do not update or create PR)")
	updateCmd.Flags().BoolVar(&draft, "draft", false, "Mark pull request as a draft (only used if a new PR is created)")
	updateCmd.Flags().BoolVarP(&debug, "debug", "d", false, "Print extra debug info (never prints secrets)")
